  ...
  return (
    <div className="flex h-dvh w-screen flex-col overflow-hidden bg-white sm:h-auto sm:max-h-[90vh] sm:w-5xl sm:max-w-xl sm:rounded-lg sm:shadow-2xl">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-stone-100 bg-white px-6 py-4">
        <div>
          <h2 className="text-xl font-bold text-stone-800">Doküman Geçmişi</h2>
        ...
        {isLoading ? (
          <div className="flex items-center justify-center py-12">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-sm font-medium text-stone-800">Dokümanlar yükleniyor...</p>
            </div>
          </div>
        ) : isError ? (
          ...
          <div className="flex flex-col items-center justify-center py-16">
            <div className="flex size-16 items-center justify-center rounded-full bg-stone-100">
              <FileText className="size-8 text-stone-800" />
            </div>
            <p className="mt-4 text-base font-medium text-stone-800">Henüz doküman yok</p>
            <p className="mt-1 text-sm text-stone-800">İlk dokümanınızı oluşturarak başlayın</p>
          </div>
        ) : (
              ...
              {data?.pages.map((page, pageIndex) => (
                <Fragment key={pageIndex}>
                  {page.items.map((item) => (
                    <div
                      key={item.id}
                      className="group relative flex flex-col items-start justify-between gap-x-6 gap-y-2 overflow-hidden rounded-lg border border-stone-200 bg-white px-4 py-4 transition-[colors_opacity] sm:flex-row sm:flex-wrap sm:items-center"
                    >
                      {/* Left */}
            ...
            {hasNextPage && (
              <div className="mt-6 flex justify-center">
                <button
                  ...
                  className="transiton-colors flex items-center gap-2 rounded-lg border border-stone-200 bg-white px-6 py-2.5 text-sm font-medium text-stone-800 duration-300 hover:border-primary hover:bg-primary hover:text-white disabled:opacity-50"
                >
                  {isFetchingNextPage ? (
                    <>
    ...
    <div className="flex h-dvh w-screen flex-col overflow-hidden bg-white sm:h-auto sm:w-xl sm:rounded-lg sm:shadow-2xl">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-stone-100 bg-white px-6 py-4">
        <div className="flex items-center gap-3">
          <div>
            <h2 className="text-lg font-bold text-stone-800">Email Bilgileri</h2>
            <p className="mt-0.5 text-xs text-stone-600">Son gönderim detayları</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="flex flex-col gap-3 p-6">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex items-start gap-3 rounded-lg border border-stone-200 bg-stone-50 p-3">
            <div className="flex min-w-0 flex-col gap-1.5">
              <p className="text-xs font-medium text-stone-600">Gönderim Tarihi</p>
              <p className="text-sm text-stone-800">{formatDate(new Date(emailMeta.sent_at))}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-lg border border-stone-200 bg-stone-50 p-3">
          <div className="flex min-w-0 flex-col gap-0.5">
            <p className="text-xs font-medium text-stone-600">Alıcı Email</p>
            <p className="truncate text-sm text-stone-800">{emailMeta.email_address}</p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-3 rounded-lg border border-stone-200 bg-stone-50 p-3">
          <div className="flex min-w-0 flex-col gap-0.5">
            <p className="text-xs font-medium text-stone-600">Email Başlığı</p>
            <p className="text-sm text-stone-800">{emailMeta.email_title}</p>
          </div>
        </div>
        ...
        {emailMeta.email_description && (
          <div className="flex items-start gap-3 rounded-lg border border-stone-200 bg-stone-50 p-3">
            <div className="flex min-w-0 flex-col gap-0.5">
              <p className="text-xs font-medium text-stone-600">Email Açıklaması</p>
      ...
      {/* Footer */}
      <div className="shrink-0 border-t border-stone-100 bg-white px-6 py-4"></div>
    </div>
  )
}
    ...
    <div className="flex h-dvh w-screen flex-col overflow-hidden bg-white sm:h-auto sm:w-sm sm:rounded-lg sm:shadow-2xl">
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-stone-100 bg-white px-6 py-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-stone-800">Not Ekle</h2>
            <p className="mt-0.5 text-xs text-stone-600">Dökümanınız için notlar oluşturun.</p>
          </div>
        </div>
        <button
          onClick={onClose}
